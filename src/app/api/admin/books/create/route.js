...
import { generateBookSlug, slugify } from "@/lib/slug";
...
export async function POST(request) {
  ...
  const categories = Array.isArray(categoriesRaw) && categoriesRaw.length > 0 
    ? categoriesRaw.map(c => normalizeString(c)).filter(c => c) 
    : (category ? [category] : []);
  const tags = Array.isArray(tagsRaw) && tagsRaw.length > 0 
    ? tagsRaw.map(t => normalizeString(t)).filter(t => t) 
    : [];
    ...
    // At least one category is required for book data integrity
    if (!categories || categories.length === 0) {
      return new Response(
        JSON.stringify({ ok: false, error: "At least one category is required" }),
        { status: 400, headers: { "content-type": "application/json" } }
      );
    }
    ...
    try {
      ...
      if (!existingAuthor) {
        ...
        const authorBaseSlug = slugify(author);
        let authorSlug = authorBaseSlug;
        let authorCounter = 1;
        
        // Ensure author slug is unique
        while (await authors.findOne({ slug: authorSlug })) {
          authorSlug = `${authorBaseSlug}-${authorCounter}`;
          authorCounter++;
        }
        
        await authors.insertOne({
          name: author,
          nameLower: author.toLowerCase(),
          slug: authorSlug,
          bio: null,
          createdAt: new Date(),
          updatedAt: new Date(),
    ...
    // If no tags provided, auto-generate them based on categories and content
    let finalTags = tags;
    if (finalTags.length === 0) {
      const autoGenerated = autoCategorizeBook({ title, author, category: categories[0], publisher, format });
      finalTags = autoGenerated.tags;
    }
    ...
    const doc = {
      ...
      categories, // Multiple categories for better organization
